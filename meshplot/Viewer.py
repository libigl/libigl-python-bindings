...
class Viewer():
    ...
    def __update_settings(self, settings={}):
        sett = {"width": 700, "height": 700, "antialias": True, "scale": 1.5, "background": "#ffffff",
                "fov": 30}
        for k in settings:
            sett[k] = settings[k]
        self.__s = sett

    def __add_object(self, obj, parent=None, update=True):
    ...
    def __add_line_geometry(self, lines, shading, obj=None):
        ...
        if obj:
            return self.__add_object(line_obj, obj, update=False), line_obj
        else:
            return self.__add_object(line_obj, update=False)
    ...
    def add_mesh(self, v, f, c=None, uv=None, shading={}, update=True):
        ...
        return self.__add_object(mesh_obj, update=update)
    ...
    def add_points(self, points, c=None, shading={}, obj=None):
        ...
        if obj:
            return self.__add_object(point_obj, obj, update=False), point_obj
        else:
            return self.__add_object(point_obj, update=False)
    ...
    def update_object(self, oid=0, vertices=None, colors=None, faces=None):
        ...
        if type(faces) != type(None): #TODO: how to update faces after generation?
            #print("Faces")
            if obj["coloring"] == "FaceColors":
            ...
            #print(obj["geometry"].attributes.keys())
